  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        <section className="space-y-6">
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                ...
                className="text-[9px] font-bold text-blue-500 hover:text-foreground disabled:opacity-30 transition-all uppercase tracking-widest"
              >
            ...
            <textarea
              ...
              className="w-full h-24 bg-background border border-border rounded-lg p-3 text-sm outline-none focus:border-blue-500 transition-all resize-none text-foreground"
              placeholder="..."
            />
          </div>
          ...
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm">
            <div className="flex justify-between items-center mb-1">
              <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{t.description}</label>
              <button
          ...
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm space-y-4">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">{t.detailImage}</h3>
              <button
                onClick={handleGenerateDetail}
                disabled={isGeneratingDetail}
                className="text-[9px] font-bold text-blue-500 hover:text-foreground transition-all uppercase tracking-widest"
              >
            ...
            <textarea
              ...
              className="w-full h-20 bg-background border border-border rounded-lg p-3 text-xs outline-none focus:border-blue-500 transition-all resize-none text-foreground"
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-background border border-border rounded-lg p-2 text-[10px] outline-none text-muted-foreground focus:border-blue-500"
          ...
          <div className="bg-card p-6 rounded-2xl border border-border shadow-sm space-y-4">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest border-b border-border pb-2">Interaction Logic</h3>
            <div>
              <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">{t.combineWith}</label>
              <select
                value={item.combinableWith || ''}
                onChange={(e) => onUpdate({ combinableWith: e.target.value || undefined })}
                className="w-full bg-background border border-border rounded-lg p-2 text-xs outline-none focus:border-blue-500 transition-all text-foreground"
              >
            ...
            <div>
              <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">{t.resultItem}</label>
              <select
        ...
        <section className="space-y-6">
          {/* Icon Preview */}
          <div className="flex flex-col items-center justify-center p-8 bg-background rounded-2xl border border-border shadow-lg relative overflow-hidden min-h-[300px]">
            <div className="w-40 h-40 rounded-3xl bg-card border border-dashed border-border overflow-hidden flex flex-col items-center justify-center relative shadow-inner">
              {!showPlaceholder ? (
                <img src={item.iconUrl} className="w-full h-full object-contain p-4 relative z-10" onError={() => setImageError(true)} alt="Icon" />
              ) : (
                <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">No Icon</div>
              )}
            </div>
            <label className="block text-[8px] font-bold text-muted-foreground uppercase tracking-widest mt-4 mb-2">{t.iconUrl}</label>
            <div className="flex gap-2 w-full mt-2">
              <input
                value={item.iconUrl || ''}
                onChange={(e) => { setImageError(false); onUpdate({ iconUrl: e.target.value }); }}
                className="flex-1 bg-card border border-border rounded-lg p-2 text-[10px] outline-none text-muted-foreground"
                placeholder="https://..."
              />
              <ImageUploader
          ...
          {/* Detail Preview */}
          <div className="flex flex-col items-center justify-center p-8 bg-background rounded-2xl border border-border shadow-lg relative overflow-hidden min-h-[300px]">
            <div className="w-full aspect-video rounded-xl bg-card border border-dashed border-border overflow-hidden flex flex-col items-center justify-center relative shadow-inner">
              {!showDetailPlaceholder ? (
                ...
                <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">No Detail Image</div>
              )}
            </div>
            <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-4">{t.detailImage}</span>
          </div>
        </section>
      </div>
    </div>
