...
const QuickModeEditor: React.FC<QuickModeEditorProps> = ({ gameData, onSave }) => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground p-4 md:p-8 lg:p-12 relative overflow-hidden transition-colors duration-300">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="mb-6">
                    <button 
                        ...
                        className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors text-sm font-bold uppercase tracking-widest"
                    >
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                    <div className="space-y-2">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-foreground to-muted">
                            바다거북스프 에디터
                        </h1>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <button
                            ...
                            className={`group relative px-6 py-4 font-bold rounded-2xl overflow-hidden transition-all duration-300 disabled:opacity-80 flex items-center justify-center gap-3 shrink-0 ${saveSuccess ? 'bg-emerald-500 text-black' : 'bg-foreground text-background shadow-lg hover:shadow-xl'}`}
                        >
                ...
                {/* AI Assistant Banner */}
                <div className="mb-8 rounded-3xl bg-card border border-border shadow-2xl relative group overflow-hidden p-0.5">
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-foreground/5 to-transparent -translate-x-[150%] skew-x-[-20deg] group-hover:animate-shimmer pointer-events-none"></div>
                    <div className="bg-card rounded-[22px] p-6 lg:p-8 flex flex-col gap-5 relative z-10">
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`group/style flex items-center gap-2 px-4 py-2 rounded-xl border font-bold text-sm transition-all duration-200
                                                ${isSelected
                                                    ? 'bg-emerald-500/20 border-emerald-500/60 text-emerald-600 dark:text-emerald-300 shadow-md'
                            ...
                            <div className="flex flex-wrap gap-2">
                                {DIFFICULTIES.map((d) => {
                                    ...
                                    const colorMap: Record<string, string> = {
                                        emerald: isSelected ? 'bg-emerald-500/20 border-emerald-500/60 text-emerald-600 dark:text-emerald-300 shadow-md' : 'bg-background border-border text-muted-foreground hover:border-emerald-500/50 hover:text-foreground',
                                        amber:   isSelected ? 'bg-amber-500/20 border-amber-500/60 text-amber-600 dark:text-amber-300 shadow-md'   : 'bg-background border-border text-muted-foreground hover:border-amber-500/50 hover:text-foreground',
                                        red:     isSelected ? 'bg-red-500/20 border-red-500/60 text-red-600 dark:text-red-300 shadow-md'         : 'bg-background border-border text-muted-foreground hover:border-red-500/50 hover:text-foreground',
                                    };
                                    return (
                                        <button
                                            key={d.key}
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-center">
                            <div className="flex-1 w-full space-y-3">
                                ...
                                <input
                                    ...
                                    className="w-full bg-transparent border-none outline-none text-xl sm:text-2xl font-bold placeholder:text-muted/50 p-0 text-foreground focus:ring-0"
                                />
                            </div>
                            <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-min">
                    ...
                    <div className="md:col-span-12 lg:col-span-4 p-1 rounded-3xl bg-card border border-border shadow-2xl relative group">
                        ...
                        <div className="h-full bg-card rounded-[22px] p-6 lg:p-8 flex flex-col gap-4 relative z-10">
                            ...
                            <input
                                ...
                                className="w-full bg-transparent border-none outline-none text-2xl font-bold placeholder:text-muted-foreground focus:ring-0 p-0 text-foreground"
                            />
                            <p className="text-muted-foreground text-sm mt-auto pt-4 border-t border-border">
                                사건의 제목을 매력적으로 지어주세요.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-12 lg:col-span-8 p-1 rounded-3xl bg-card border border-border shadow-2xl relative group">
                        ...
                        <div className="h-full bg-card rounded-[22px] p-6 lg:p-8 flex flex-col md:flex-row gap-6 relative z-10">
                            
                            {/* Image Preview Window */}
                            <div className="w-full md:w-1/2 rounded-xl bg-background border border-border relative flex items-center justify-center overflow-hidden shrink-0">
                                {imageUrl ? (
                                    <img src={imageUrl} alt="Generated Preview" className="w-full h-full object-cover" />
                                ) : (
                                    <div className="text-center text-muted-foreground flex flex-col items-center gap-2 py-12">
                             ...
                             <div className="flex-1 flex flex-col gap-4">
                                ...
                                <textarea
                                    ...
                                    className="w-full h-full bg-transparent border-none outline-none text-sm text-foreground/80 placeholder:text-muted-foreground resize-none focus:ring-0 p-0 font-pretendard"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-12 xl:col-span-6 p-1 rounded-3xl bg-card border border-border shadow-2xl relative group md:min-h-[320px]">
                        ...
                        <div className="h-full bg-card rounded-[22px] p-6 lg:p-8 flex flex-col gap-4 relative z-10">
                            ...
                            <textarea
                                ...
                                className="w-full bg-transparent border-none outline-none text-xl sm:text-2xl font-pretendard text-foreground placeholder:text-muted-foreground resize-none flex-1 focus:ring-0 p-0 leading-relaxed"
                            />
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-12 xl:col-span-6 p-1 rounded-3xl bg-card border border-border shadow-2xl relative group md:min-h-[320px]">
                        ...
                        <div className="h-full bg-card rounded-[22px] p-6 lg:p-8 flex flex-col gap-4 relative z-10">
                            <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center justify-between">
                                ...
                                <span className="text-[10px] bg-red-500/10 text-red-600 border border-red-500/20 px-2 py-1 rounded-lg">1급 기밀</span>
                            </label>
                            <textarea
                                value={hiddenTruth}
                                onChange={(e) => setHiddenTruth(e.target.value)}
                                placeholder="남자는 과거 무인도에 조난되었을 때 맹인 상태였고... (AI가 사용자 질문에 Yes/No를 판단하는 절대 기준이 됩니다.)"
                                className="w-full bg-transparent border-none outline-none text-lg text-foreground/80 placeholder:text-red-900/40 resize-none flex-1 focus:ring-0 p-0 leading-relaxed font-pretendard"
                            />
                        </div>
                    </div>
